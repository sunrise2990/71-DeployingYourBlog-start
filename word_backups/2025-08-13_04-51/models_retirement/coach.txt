...
def _solve_single_lever(params: Dict[str, Any],
    ...
    async_like = False  # this runs synchronously; kept for symmetry
    ...
    # bracket & quick checks
    f_lo = eval_assets(lo) - target_assets
    f_hi = eval_assets(hi) - target_assets

    # If lever is increasing and low end already meets target, choose low.
    # If decreasing and high end meets, choose high.
    feasible = True
    if inc:
        if f_lo >= 0:
            hi = lo
...
def coach_suggestions(params: Dict[str, Any],
    ...
    if target and "type" in target:
        ...
        if t == "p10_nonneg":
            ...
            target = {"metric": "p10", "assets": 0.0, "age": int(target.get("age", params.get("life_expectancy", 90)))}
            lever = (lever or (prefs.get("levers", ["retirement_age"])[0]))
        elif t == "final_assets_min":
            target = {"metric": "p50", "assets": float(target.get("amount", 0.0)), "age": int(target.get("age", params.get("life_expectancy", 90)))}
            lever = (lever or (prefs.get("levers", ["retirement_age"])[0]))
    ...
    if target and lever:
        ...
        age_goal = int(target.get("age", params.get("life_expectancy", 90)))
        b = bounds_all.get(lever)
        x_star, patch, achieved = _solve_single_lever(params, metric, assets_goal, age_goal, lever, tuple(b) if b else None)
