...
migrations_dst = backup_dir / "migrations"
if migrations_src.exists() and migrations_src.is_dir():
    shutil.copytree(migrations_src, migrations_dst)
    print(f"📁 Recursively copied migrations folder → {migrations_dst}")
else:
    print(f"⚠️ Migrations folder not found: {migrations_src}")

# === STEP 7: Delete old folders, keeping 2 most recent ===
