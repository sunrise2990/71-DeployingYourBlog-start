...
# 🔄 Monte Carlo — ONLY randomizes return and inflation
    ...
    *,
    ...
    for s in range(num_simulations):
        ...
        for idx, age in enumerate(ages):
            rand_return = np.clip(np.random.normal(return_mean, return_std), -0.2, 0.12)
...
def _compute_depletion_probabilities(sim_paths: np.ndarray, start_age: int, checkpoints: list[int]):
    """Return probability of hitting zero on or before each checkpoint age, plus 'ever'."""
