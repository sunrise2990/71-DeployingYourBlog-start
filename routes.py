...
from models.retirement.retirement_calc import (
    ...
    run_mc_with_seed,  # ★ for stable MC
)
...
def _fix_std_fields_inplace(p: dict) -> dict:
    """
    Convert % → decimals for std-dev fields if needed, IN-PLACE.
    Called before any μ computations and before persisting/loading scenarios.
    """
    try:
        if "return_std" in p and p["return_std"] is not None:
            v = float(p["return_std"])
            if v > 1.0:  # 8 -> 0.08
                p["return_std"] = v / 100.0
    except Exception:
        pass
    try:
        if "inflation_std" in p and p["inflation_std"] is not None:
            v = float(p["inflation_std"])
            if v > 1.0:  # 2 -> 0.02
                p["inflation_std"] = v / 100.0
    except Exception:
        pass
    return p
...
def to_canonical_inputs(raw: dict) -> dict:
    ...
    # ★ Ensure std-devs are decimals before we ever compute μ or save to DB
    _fix_std_fields_inplace(p)
...
def canonical_to_form_inputs(canon: dict) -> dict:
    ...
    # Optional MC fields (already decimals)
    if "return_std" in d:
        ...
        if action == "reset":
            ...
            try:
                ...
                baseline_params = {
                    ...
                    "annual_saving": monthly_saving * 12,  # monthly -> annual
                ...
                # --- Main projection run (deterministic uses CAGR) ---
                ...
                # --- Monte Carlo: arithmetic μ so MC median ~ deterministic (CAGR) ---
                mu_pre  = _arith_from_cagr(return_rate,       return_std)
                mu_post = _arith_from_cagr(return_rate_after, return_std)

                seed = _get_or_create_seed()  # ★ stable across page loads
                mc_args = dict(
                    current_age=current_age,
                    retirement_age=retirement_age,
                ...
                mc_output = run_mc_with_seed(seed, run_monte_carlo_simulation_locked_inputs, **mc_args)
...
def save_scenario():
    ...
    # If a raw form payload, convert monthly savings to annual before canonicalizing
    try:
    ...
    # Normalize → canonical (also fixes std fields to decimals)
...
def load_scenario(scenario_id):
    ...
    # Ensure decimals in memory too
    fixed = to_canonical_inputs(scenario.inputs_json or {})
    form_inputs = canonical_to_form_inputs(fixed)
...
def _mc_args_from_params(p):
    # ★ make a working copy and ensure stds are decimals BEFORE μ calc
    p = dict(p or {})
    _fix_std_fields_inplace(p)

    std = float(p.get("return_std", 0.08) or 0.08)
    ...
    return {
        ...
        "inflation_std":        float(p.get("inflation_std", 0.005) or 0.005),
...
def compare_retirement():
    """
    Compare Monte Carlo between two saved scenarios (union axis, padded series).
    """
...
def _build_mc_args(p, n_sims):
    # ★ ensure stds are decimals BEFORE μ calc
    p = dict(p)
    _fix_std_fields_inplace(p)
...
def _solve_single_lever(p0, prefs, seed, _DET, _MC, run_mc_with_seed):
    ...
    if base_val is not None and base_val >= want_assets:
        return {"patch": {}, "achieved": float(base_val), "lever_value": p0.get(lever),
...
def live_update():
    ...
    def _defaults():
        return dict(
            current_age=53, retirement_age=65,
            annual_saving=48000, saving_increase_rate=0.02,
            current_assets=850000, annual_expense=72000,
            cpp_monthly=1200, cpp_start_age=65, cpp_end_age=70,
            asset_liquidations=[],
            inflation_rate=0.025, life_expectancy=92, income_tax_rate=0.15,
            return_rate=0.065, return_rate_after=0.045,
            return_mean=0.065, return_mean_after=0.045,
            return_std=0.10, inflation_mean=0.025, inflation_std=0.01,
